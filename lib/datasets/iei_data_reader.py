...
import scipy.sparse
import cPickle
from iei_eval import iei_eval, load_truth_from_xmlnode, get_files
...
class iei_data_reader(imdb):
    ...
    def _load_image_set_index(self):
        ...
        """
        # get index to image path dict
        image_index_to_path = {}
        index = 0
        for image_path in get_files(self._data_path, self._image_ext):
            image_index_to_path[str(index)] = image_path
            index += 1
        self._image_index_to_path = image_index_to_path

        return [str(x) for x in xrange(len(image_index_to_path))]
    ...
    def image_path_from_index(self, index):
        ...
        """
        return self._image_index_to_path[index]
    ...
    def _load_iei_annotation(self, index):
        ...
        format.
        """
        filename = self.image_path_from_index(index).replace(self._image_ext, self._label_ext)
        print 'load iei label from file', filename
                ...
                for im_ind, index in enumerate(self.image_index):
                    ...
                    for k in xrange(dets.shape[0]):
                        f.write('{:s} {:.3f} {:.1f} {:.1f} {:.1f} {:.1f}\n'.
                                format(os.path.basename(self.image_path_from_index(index).replace(self._image_ext, self._label_ext)), dets[k, -1],
